...
namespace RentACar.Api.Controllers
{
    ...
    public class AraclarController : ControllerBase
    {
        ...
        //marka ile çağırmak için get metodu oluşturduk.

        [HttpGet("marka")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Arac))]
        public IActionResult Get([FromQuery]string marka)
        {
            var arac = _context.Araclar.SingleOrDefault(p => p.marka == marka);
            if (arac == null)
                return NotFound();
            return Ok(arac);
        }
